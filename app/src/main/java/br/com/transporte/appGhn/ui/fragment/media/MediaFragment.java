...
public class MediaFragment extends Fragment {
    ...
    private void carregaAbastecimentos() {
        ...
        observer.observe(getViewLifecycleOwner(),
                new Observer<List<CustosDeAbastecimento>>() {
                    @Override
                    public void onChanged(List<CustosDeAbastecimento> resource) {
                        observer.removeObserver(this);
                        enviaDataParaView(resource);
                        bindPlaca();
                    }
                });
    }
    ...
    private void enviaDataParaView(
            @NonNull final List<CustosDeAbastecimento> listaAbastecimento
    ) {
        listaAbastecimento.sort(Comparator.comparing(CustosDeAbastecimento::getMarcacaoKm));
        Collections.reverse(listaAbastecimento);
    ...
    private void configuraBtnFiltrar() {
        btnFiltrar.setOnClickListener(v -> {
            final CustosDeAbastecimento flag1 = adapterDeAbastecimentoHelper.getFlagAbastecimento1();
            final CustosDeAbastecimento flag2 = adapterDeAbastecimentoHelper.getFlagAbastecimento2();

            final MediaBottomDialog dialog =
                    new MediaBottomDialog(requireContext(), flag1, flag2, viewModel, this);
    ...
    private void configuraAdapterDeAbastecimentoHelper() {
        adapterDeAbastecimentoHelper = new MediaAdapterAbastecimentoHelper(this.requireContext(), recyclerDeAbastecimentos);
    ...
    private void configuraAdapterDeCavaloHelper() {
        ...
        adapterDeCavaloHelper = new MediaAdapterCavaloHelper(requireContext(), new ArrayList<>(), viewModel.getCavaloSelecionado(), recyclerDeCavalos);
