...
public class ComissoesAdapter extends RecyclerView.Adapter<ComissoesAdapter.ViewHolder> {
    ...
    public ComissoesAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        final View viewCriada = LayoutInflater.from(context.getContext()).inflate(R.layout.recycler_item_comissoes, parent, false);
    ...
    public void onBindViewHolder(@NonNull ComissoesAdapter.ViewHolder holder, int position) {
        final Cavalo cavalo = dataSet_cavalo.get(position);
...
class GeraValoresParaUi {
    ...
    public Resource run(@NonNull final Cavalo cavalo) {
        ...
        final BigDecimal comissaoEmAberto = getComissaEmAberto(dataSet);
        final String comissaoEmAberto_string = FormataNumerosUtil.formataMoedaPadraoBr(comissaoEmAberto);
        resource.setComissaoEmAberto(comissaoEmAberto_string);

        final BigDecimal comissaoPaga = getComissaoJaPaga(dataSet);
        final String comissaoPaga_string = FormataNumerosUtil.formataMoedaPadraoBr(comissaoPaga);
        resource.setComissaoPaga(comissaoPaga_string);

        final BigDecimal adiantamento = getAdiantamentoADescontar(cavalo.getId());
        final String adiantamento_string = FormataNumerosUtil.formataMoedaPadraoBr(adiantamento);
        resource.setAdiantamento(adiantamento_string);

        final BigDecimal reembolso = getReembolsoDevidoAoMotorista(cavalo.getId());
        final String reembolso_string = FormataNumerosUtil.formataMoedaPadraoBr(reembolso);
        resource.setReembolso(reembolso_string);

        final String nome = getNomeMotorista(cavalo.getRefMotoristaId());
