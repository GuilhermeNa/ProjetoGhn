...
public class FormularioDespesaAdmFragment extends FormularioBaseFragment {
    ...
    private void configuraDropDownMenuDePlacas() {
        ...
        final ArrayAdapter<String> adapterCavalos =
                new ArrayAdapter<>(this.requireContext(), android.R.layout.simple_list_item_1, cavalos);
    ...
    public void exibeObjetoEmCasoDeEdicao() {
        try {
            if (viewModel.getTipoDespesa() == DIRETA) {
                final Frota cavalo = FiltraCavalo.localizaPeloId(
                        viewModel.getListaDeCavalos(),
                        viewModel.despesaArmazenada.getRefCavaloId());
                String placa = "";
                if (cavalo != null) {
